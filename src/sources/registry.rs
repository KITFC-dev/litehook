...
pub struct SourceRegistration {
    pub kind:    &'static str,
    pub factory: fn(SourceConfig, mpsc::Sender<Event>) -> Pin<Box<dyn Future<Output = anyhow::Result<Box<dyn Source>>>>>,
}

inventory::collect!(SourceRegistration);

/// Build a source from config
pub async fn build(cfg: SourceConfig, tx: mpsc::Sender<Event>) -> anyhow::Result<Box<dyn Source>> {
